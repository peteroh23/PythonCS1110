...
class Touch(GameApp):
    ...
    INSTANCE ATTRIBUTES (Not hiding any):
        ...
        clicks: the # of fast clicks in a row   [int >= 0]: 
        lines:  the set of lines to be drawn    [list of GLine]
        last:   the last mouse position
                [GPoint or None if mouse was not down last frame]:
        time:   the frames since mouse last changed (pressed, released) 
                [int >= 0, 0 if mouse state changed]:
    """
    ...
    def _drawLines(self):
        ...
        line = GPath(points=points,linecolor=cornell.RED)
